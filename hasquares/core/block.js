...
Blockly.Block.prototype.reforgeUp = function() {
  for (var i = 0, child; child = this.childBlocks_[i]; i++) {
      if (child.outputConnection) {
        console.log("reforging up");
      }
      child.reforgeUp();
      this.reforge(otherBlock, locationSelf, -1)
  }
}

Blockly.Block.prototype.reforgeDown = function() {
  for (var i = 0, child; child = this.childBlocks_[i]; i++) {
      if (child.outputConnection) {
        console.log("reforging down");
      }
      child.reforgeDown();
      this.reforge(otherBlock, -1, locationOther)
  }
}

Blockly.Block.prototype.reforge = function(otherBlock, locationSelf, locationOther) {

}


Blockly.Block.prototype.setBlockClass = function (thisClass) {
  this.blockClass = thisClass
}

Blockly.Block.prototype.setOriginClass = function (thisClass) {
  this.originClass = thisClass
}

Blockly.Block.prototype.trueContext = function (truecontext) {
  this.savedContext = truecontext
}

Blockly.Block.prototype.functionContext = function (context) {
  this.currentContext = context
  if (this.savedContext[0][0] == this.currentContext[0][0] &&
      this.savedContext[1][0] == this.currentContext[1][0]) {
  }
  else {
    this.setOutput(true, this.currentContext[1][0])
  }
}
